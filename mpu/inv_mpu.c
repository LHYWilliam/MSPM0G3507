 ...
 *  \n MPU6500: 0.24Hz, 0.49Hz,
 * 0.98Hz, 1.95Hz, 3.91Hz, 7.81Hz, 15.63Hz, 31.25Hz, 62.5Hz, 125Hz, 250Hz, 500Hz
...
int mpu_reset_fifo(void) {
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &data))
            return -1;
        if (i2c_write(st.hw->addr, st.reg->fifo_en, 1,
 ...
 *  @return     0 if successful.
 */
int mpu_set_compass_sample_rate(unsigned short rate) { return -1; }
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test) {
    ...
    gyro[0] =
        (long)(((long long)gyro[0] << 16) / test.gyro_sens / packet_count);
    gyro[1] =
        (long)(((long long)gyro[1] << 16) / test.gyro_sens / packet_count);
    gyro[2] =
        (long)(((long long)gyro[2] << 16) / test.gyro_sens / packet_count);
    accel[0] =
 ...
 *  @return     0 if successful.
 */
int mpu_get_compass_fsr(unsigned short *fsr) { return -1; }
 ...
 *  \n 0.24Hz, 0.49Hz, 0.98Hz, 1.95Hz, 3.91Hz, 7.81Hz, 15.63Hz, 31.25Hz, 62.5Hz,
 * 125Hz, 250Hz, 500Hz
 *
 *  \n\n NOTES:
